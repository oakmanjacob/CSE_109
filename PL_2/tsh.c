...
int main(int argc, char **argv)
{
	...
	/**
	 * Check for input flags when tsh is called
	 * -h   print a help message (this also triggers on non recognized flags)
...
void eval(char *cmdline)
{
	...
	if (argv[0] != NULL && !builtin_cmd(argv))
	{
		...
		{
			if (bg) // is background job
			{
				addjob(jobs, pid, BG, cmdline);
				sigprocmask(SIG_UNBLOCK, &mask, NULL);
			}
			else // is foreground job
			{
...
void sigint_handler(int sig)
{
	...
	{
		printf("There is no current foreground process!\n");
	}
	return;
}
